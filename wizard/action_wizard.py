...
class ServerActionWizardStock(models.TransientModel):
    ...
    def approve_marketplace_stocks(self):
        if self.marketplace_stock_ids:
            msg = "<p style='font-size: 15px'>Inventory for selected product(s) can't be approve, only pending inventory request(s) will get approve.<p>"
            ...
            if approved_inventory:
                ...
                msg = "<p style='font-size: 15px'>Inventory request for product(s) approved successfully: <strong>" + prod_id + "</strong></p>"
            return self.env["mp.wizard.message"].generated_message(msg, "Approved Status")


    def reject_marketplace_stocks(self):
        msg = "<p style='font-size: 15px'>Inventory for selected product(s) can't be reject.<p>"
        if self.marketplace_stock_ids:
            ...
            if rejected_inventory:
                ...
                msg = "<p style='font-size: 15px'>Inventory request for product(s) rejected successfully: <strong>" + prod_id + "</strong></p>"
            return self.env["mp.wizard.message"].generated_message(msg, "Rejected Status")
